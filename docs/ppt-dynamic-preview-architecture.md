...
当前项目是 MVP 形态，核心链路已经打通，但内容结构、版式系统、动态展示和导出能力都还比较基础。后续升级时建议优先围绕“数据协议先行”展开：先扩展 `types.ts` 中的 PPT 页面表达能力，再同步改大模型 prompt、渲染器、动画帧、前端展示和 PPTX 导出。
...
前端当前主要负责表单、模板选择、SSE 播放和简单预览。后续如果要接近真实 PPT 产品，需要增强编辑、预览和交互能力：

- **生成参数面板**：增加页数、受众、语气、行业、汇报场景、详细程度、是否包含图表、是否包含案例等参数。
- **大纲确认流程**：先展示大模型生成的大纲，让用户确认或修改后再生成完整 PPT。
- **逐页编辑能力**：支持用户在前端修改标题、副标题、bullet、备注，再重新渲染和导出。
